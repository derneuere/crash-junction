...
/** The real-recording engine: three steady RPM holds looped seamlessly and
 *  crossfaded/pitch-bent across the virtual-gear band, with a clean synth
 *  sub underneath for chest. Falls back to (and upgrades from) the
 *  EngineSynth while the clips decode. */
export class EngineSound {
  ...
  constructor(
    ...
    private out: AudioNode,
    private noise: AudioBuffer,
    private getClip: (name: EngineLayerName | 'shift') => EngineClip | null,
  ) {
  ...
  private tryBuild(): boolean {
    const clips = ENGINE_LAYERS.map((l) => this.getClip(l.name));
  ...
  update(speed: number, vol: number, throttle: boolean, warp: number): void {
    ...
    this.sub.frequency.setTargetAtTime((38 + 120 * vrpm * vrpm) * 0.5 * warp, t, 0.04);
    this.gain.gain.setTargetAtTime(vol, t, 0.08);
