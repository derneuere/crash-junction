...
import { duneTufts, fenceLine } from './beach/scatter';
import { parkingBays } from './beach/parking';
...
export const beach: ZoneDressing = {
  props: [
    ...
    // [art-grass-sand] dense dune-grass tuft scatter down the whole lip,
    // tracking the sand patch's inland boundary (beach.patches[0] first ring)
    // ~2-3 m onto the grass side so the tufts sprout where lawn meets sand.
    // The NW reach (first segments) is the locked refshot pose's foreground.
    ...duneTufts(
      [
        [-238, -94],
        [-222, -132],
