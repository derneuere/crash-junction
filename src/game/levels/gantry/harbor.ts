...
// only (decal slot 0.014); the actual barrier is the kerb wall style below.
...
const BOLLARDS: PropDef[] = alongLip(13, 1.05, 6)
  ...
  .map((p) => decor('builtin:bollard', p.x, p.z, 0, 1.5));
...
export const harbor: ZoneDressing = {
  props: [
    ...
    { url: `${DOCK}/crane-harbor-hancock.glb`, x: 282, z: 140, yaw: -0.9, scale: 0.3, y: -1.0, tint: 0xff7014, collider: { hx: 7, hy: 5.5, hz: 7 } },
    ...
    decor('builtin:lamp-post', 259, 97, Math.PI / 2, 1.15),
    decor('builtin:lamp-post', 266, 128, Math.PI / 2, 1.15),
    decor('builtin:lamp-post', 271, 152, Math.PI / 2, 1.15),
    decor('builtin:lamp-post', 276, 180, Math.PI / 2, 1.15),
    ...
    // pile reads as coiled mooring rope at quay distance.
    decor(`${CARGO}/crate.glb`, 262, 103, 0.4, 1.15),
    decor(`${CARGO}/crate.glb`, 263.8, 105.6, -0.2, 0.95, { y: 0 }),
    decor(`${CARGO}/tires.glb`, 260, 106, 1.1, 1.15),
    decor(`${CARGO}/pallet.glb`, 265, 101.5, 0.9, 1),
    ...
    decor(`${CARGO}/crate.glb`, 272, 106, 0.7, 0.7),
    decor(`${COAST}/life-preserver.glb`, 272, 106, 0.3, 0.0042, { y: 0.95 }),
    // second cluster up the quay, by the crane's working radius
    decor(`${CARGO}/crate.glb`, 269, 157, -0.5, 1.25),
    decor(`${CARGO}/wheels-stack.glb`, 271.2, 159.5, 0.8, 1.9),
    decor(`${CARGO}/pallet.glb`, 267, 159.8, 0.2, 1),
  ],
  ...
  // show); points sampled from the resampler at centre + 8 m on sections
  // 76..89. The east bound IS the lip, so the apron meets the wall skirt
  ...
  // infield like the concept. Sections 76..89: quay rejoin to the z=185
  // cliff handoff, indices from the race.ts resampler math (the audit
  // script's catmull/resample replica — N=220). The second entry wins over
  // the first (last-wins within the array): the port's chain-link keeps the
  // INLAND side through 82, because the dockyard's port-exit container row
  // runs right behind that wall up to the box at (222,124) ≈ section 82 —
  // the fence ends at the last container, and only then does the kerb ring
  // the open grass infield. The seaward side goes kerb at 76 with the
  // apron+stripe, so each style change lands on a readable landmark.
  wallStyles: [
    { from: 76, to: 89, side: 'both', style: 'kerb' },
  ...
  coast: [
    { x: 286, z: 176, edge: 'wall' }, // fishing-boat-green floats just east of here
    { x: 290, z: 148, edge: 'wall' },
